...
class gateway extends dbobject
{
    public function registerUser($data)
    {
        ...
        if ($data['operation'] != 'edit') {
            ...
            if (!$validation['error']) {
                $payload = [
                    "token" => "WWtOVURsQWl0QlcwNzlYeEZoeVk4a2JPcUphVXdPVDVObUV6ckNYZ3FuZEJoYXM3dU95RjJSUG56WGFKYmtOU0Q1N2ZSZnRIME9FUjhSWERvV3VqaFUxYnFkVm5DVDlub0NReE5aa0d2NzVqNkYvQUxENi9RdTgwRTR3VGxvSll4ZGJGZmFpOVJjMzY4S0pnZWVzTm5meU5tL2VmQUNKVXNVUUtLczhpSDU0PTo6xLYp7JuA",
                    "operation" => "edit",
                    "password" => $data['password'],
                    "role_id" => $data['role_id'],
                    "firstname" => $data['firstname'],
    ...
    function getUserList($data)
    {
        ...
        // $logData = json_encode($response, JSON_PRETTY_PRINT);
        // // $logData = json_decode($response, true);

        // // Define the log file path
        // $logFile = 'logs/payload_log.txt';
    ...
    {
        // Check if 'route' key is present
        $routeCheck = $this->checkRoute($payload);
        if ($routeCheck) {
            return $routeCheck;  // Return error if 'route' is missing
        }
        $route = $payload['route'];
        ...
        curl_setopt_array($curl, array(
            ...
            CURLOPT_HTTPHEADER => array(
                'Content-Type: application/json',
                // 'Authorization: Bearer '.$header['token'].'' // Add token if needed
            ),
        ));
    ...
    public function dropCookie($data)
    {
        ...
        // $_SESSION['userRecord'] = $cookieValue;
        setcookie('userRecord', $cookieValue, time() + 3600, '/');
    ...
    public function setSession($data)
    {
        ...
        $_SESSION['role_id_sess'] = $data['role_id_sess'];
        $_SESSION['username_sess'] = $data['username_sess'];
        $_SESSION['firstname'] = $data['firstname'];
        $_SESSION['lastname'] = $data['lastname'];
        $_SESSION['facilityName'] = $data['facilityName'];
        $_SESSION['facilityId'] = $data['facilityId'];
    }
