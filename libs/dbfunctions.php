...
*/
	...
	function getcount($sql)
	{
		...
		$query = $sql;
		//echo $query;
		file_put_contents("counter.txt", $query);
		$result = mysqli_query($this->myconn, $query); //or die(mysql_error());
		$numrows = mysqli_num_rows($result);
		if ($numrows > 0) {
			$row = mysqli_fetch_array($result);
	...
	public function sendAPIEmail($to, $subject, $template_file)
    {

        // passing true in constructor enables exceptions in PHPMailer
        $mail = new PHPMailer(true);

        $dee_name = 'CERBERUS MICROSERVICES';
        ...
        try {
            ...
            //Recipients
            $mail->setFrom('ezekialafolabi11@gmail.com', 'CERBERUS MICROSERVICES');
            // $mail->addAddress('ezekielafolabi22@gmail.com');
            $mail->addAddress($to, $dee_name);

    
            $mail->isHTML(true);
			$mail->AddEmbeddedImage('../../img/logo.png', 'logo');                                  //Set email format to HTML
            $mail->Subject = $subject;
            $mail->Body    = $template_file;
            $mail->AltBody = 'CERBERUS SERVICES';

            if ($mail->send()) {
                return json_encode(array('response_code'=>0, 'response_message'=>'Email Sent successfully'));
            } else {
                return json_encode(array('response_code' => 20, 'response_message' => 'Message not sent'));
            }
        } catch (Exception $e) {
            return json_encode(array('response_code' => 20, 'response_message' => "Message could not be sent. Mailer Error: {$mail->ErrorInfo}"));
        }
    }
