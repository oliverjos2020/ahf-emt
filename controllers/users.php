...
class Users extends dbobject
{
    public function login($data)
    {
        ...
        $sql = "SELECT role_id,username,firstname,lastname,password,user_locked,user_disabled,pin_missed,day_1,day_2,day_3,day_4,day_5,day_6,day_7 FROM userdata WHERE username = '$username'";
        // return $sql;
                        ...
                        if ($is_locked != 1) {
                            ...
                            if ($work_day['code'] != "44") {
                                //update pin missed and last_login
                                    ...
                                    // $getRoleID = $this->getitemlabel('user_role', 'username', $username, 'role_id');
    ...
    public function userlist($data)
    {
        ...
        $columner = array(
            ...
            array(
                ...
                'formatter' => function ($d, $row) {
                    // return $this->getitemlabel('user_role', 'user_id', $row['email'], 'role_id');
                    // return $this->getItemLabelArr('user_role', '["user_id"]', [$row['email']], 'role_id');
                    $sql = "SELECT role_id FROM user_role WHERE user_id = '$row[email]'";
                    $query = $this->db_query($sql, true);
                    // for($i = 0; $i < count($query[0]); $i++){
                    //     echo $this->getitemlabel('role', 'role_id', $i['role_id'], 'role_name');
                    // }
                    $result = "";
                    if(is_array($query)):
                    foreach ($query as $item) {
                        if (isset($item['role_id'])) {
                            $result .= $this->getitemlabel('role', 'role_id', $item['role_id'], 'role_name').", ";
                        }
                    }
                    endif;
                    return $result;
                    // print_r($query);
                    // $result = "";
                    // foreach ($query[0] as $r){
                    //     echo $this->getitemlabel('role', 'role_id', $r, 'role_name');
                    // }
                    // return $result;
                }
            ),
            ...
            array(
                ...
                'formatter' => function ($d, $row) {
                    $locking = ($row['user_disabled'] == 1) ? "Enable" : "Disable";
                    $locking_class = ($row['user_disabled'] == 1) ? "btn-success" : "btn-danger";
                        return "<a class='btn btn-sm btn-success' onclick=\"myLoadModal('modules/user/user.php?op=edit&username=" . $d . "','modal_div')\"  href=\"javascript:void(0)\" data-toggle=\"modal\" data-target=\"#defaultModalPrimary\" >Edit</a>
                        ";
        ...
        // if ($_SESSION['role_id_sess'] == "029"):
        //     $filter .= " AND role_id IN ('0011','012')";
        // endif;
    ...
    public function generatePwdLink($data)
    {
        ...
        $sql = "SELECT username,email,lastname FROM userdata WHERE username = '$username'";
        $rr = $this->db_query($sql);
        if (is_array($rr) and count($rr) > 0) {
            // 
            if (filter_var($rr[0]['email'], FILTER_VALIDATE_EMAIL)) {
                ...
                <div style="width:500px; background:white; margin:0px auto; padding:25px;">
                    ...
                    <p style="text-align:center;">Dear ' . $rr[0]['lastname'] . ', To reset your password kindly follow the link below <br><center>https://techhost7x.accessng.com/anpr_fct/pwd_reset.php?ga=' . $link . ' </center><br><center>Or copy and paste the link below in any web browser to activate your account</center></p>
                </div>
    ...
    public function register($data)
    {
        ...
        if ($data['operation'] != 'edit') {
            ...
            if (!$validation['error']) {
                ...
                // $church_id = isset($data['church_id']) ? $data['church_id'] : 0;
                // $datas['username'] = $data['username'];
                // $datas['role_id'] = $data['role_id'];
                // $data['user_locked'] = $data['is_locked'];
                unset($data['reg_status']);
                unset($data['reg_type']);
                unset($data['web_look_up']);
                unset($data['passch_logon']);
                unset($data['is_locked']);
                // unset($data['role_id']);
                // print_r($data); exit;

                $sql = "SELECT * FROM userdata WHERE username = '$data[username]' LIMIT 1";
                $role_cnt = $this->db_query($sql, false);
                if ($role_cnt < 1) {
                    $count = $this->doInsert('userdata', $data, array('op', 'confirm_password', 'operation', 'nrfa-csrf-token-label','role_id'));
                    if ($count == 1) {
                        foreach($data['role_id'] as $role){
                            $arr['user_id'] = $data['email'];
                            $arr['role_id'] = $role;
                            $this->doInsert('user_role', $arr, []);
                        }
                        // $this->doInsert('user_role', $datas, []);
                    ...
                    return json_encode(array("response_code" => 20, "response_message" => "credentials already exist"));
                }
            } else {
        ...
        $data['role_id'] = (isset($data['role_id'])) ? $data['role_id'] : '';
            ...
            if (!$validation['error']) {
                ...
                // $data['modified_date'] = date('Y-m-d H:i:s');
                // $data['modified_by'] = $_SESSION['username_sess'];
                //   unset($data['role_id']);
                unset($data['reg_status']);
                unset($data['reg_type']);
                unset($data['web_look_up']);
                ...
                // unset($data['is_locked']);
                // $data['user_locked'] = $data['is_locked'];
                    $count = $this->doUpdate('userdata', $data, array('op', 'operation', 'password', 'nrfa-csrf-token-label','modified_date','is_locked', 'role_id'), array('username' => $data['username']));
                // if ($count == 1) {
                    //                    rename('user_passport/'.$temp_pass,'user_passport/'.$data['email'].".".end($array));
                    foreach($data['role_id'] as $role){
                        $arr['user_id'] = $data['username'];
                        $arr['role_id'] = $role;
                        $this->doInsert('user_role', $arr, []);
                    }
                    return json_encode(array("response_code" => 0, "response_message" => 'Record updated successfully'));
                // } else {
                //     return json_encode(array("response_code" => 78, "response_message" => 'No changes were detected'));
                // }
            } else {
                return json_encode(array("response_code" => 20, "response_message" => $validation['messages'][0]));
            }
    ...
    public function emailPasswordChange($data)
    {
            $validation = $this->validate($data,
                        ...
                        array('username'=>'Username','confirm_password'=>'Confirm password','password'=>'Password')
                       );
...
//              
                    ...
                    $cipher_password = $desencrypt->des($key, $user_password, 1, 0, null,null);
                    $str_cipher_password = $desencrypt->stringToHex ($cipher_password);
                    $query_data = "UPDATE userdata set password='$str_cipher_password', passchg_logon = '0', reset_pwd_link = NULL WHERE username = '$username'";
//                    echo $query_data;
                    $result_data = $this->db_query($query_data,false);
                    if($result_data > 0)
                    {
